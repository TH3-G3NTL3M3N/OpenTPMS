...
- **Mounting (Outrider-style stem clamp, clarified 2026-08-13):** Requires a tubeless valve with a REMOVABLE rim seal (CushCore, Peaty's MK2, Reserve Fillmore, Outrider valves — NOT Stan's/DT Swiss, whose cones are molded on). Slide the cone off, pass the stem through the PCB hole, refit the cone. External locknut clamps the stack: valve head → PCB → (optional spacer) → cone → rim. The cone seals the rim hole exactly as stock; the PCB floats ~1mm above the rim bed and is not part of the air seal. ⚠ Rev 1 boards have a Ø6.0mm routed hole. Measured presta stems are often ~5.8mm and fit as-is (verified 2026-08-24) — test-fit the actual removable-seal valve; only if it binds, hand-file toward Ø6.7mm (nearest via is 4.2mm from center, so safe). Coat the bore either way. ⚠ Valve heads (~9-10mm) overlap the module pad row (0.8mm from hole edge) — Rev 1 clamps via a printed 9→7mm taper adapter (ABS/ASA, ≥2.5mm tall, bearing face ≤Ø7.4) or a spare rubber seal as top cushion; never let the bare head land on solder fillets. Rev 2: Ø6.70 hole + bare Ø12 clamp annulus (no pads/traces/bodies).
